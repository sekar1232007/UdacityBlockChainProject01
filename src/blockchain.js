...
class Blockchain {
    ...
    constructor() {
        this.chain = [];
        this.height = -1;
        console.log("Blockchain Init Called");
    ...
    async initializeChain() {
        if( this.height === -1){
            ...
            let temp_var = await this._addBlock(block);
            //console.log ("temp_var is : ",temp_var);
            //console.log("Validate Genesis Block ",block.validate());
            //console.log("decodeData",block.getBData());
            
            // Following code is for unit testing:
            let block1 = new BlockClass.Block({data: 'First Block'});
            await this._addBlock(block1);
            console.log("Validate Block 1:",block1.validate());
            console.log("decodeData",block1.getBData());
            let block2 = new BlockClass.Block({data: 'Second Block'});
            await this._addBlock(block2);
            console.log("Validate Block 2",block2.validate());
            console.log("decodeData",block2.getBData());
        }
    }
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            ...
            resolve(address.toString()+':'+ time.toString() +':starRegistry');
        });
    }
    ...
    getBlockByHeight(height) {
        let self = this;
        return new Promise((resolve, reject) => {
            console.log("Inside height",height,this.height);
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(flag){
                console.log("Validate Chain Pass");
                resolve(true);
            }   
            else{
